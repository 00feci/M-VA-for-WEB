...
// Az 'A', 'Ü', '-' kódok törlést váltanak ki (felszabadítják a helyet a rendszer/fejléc adatnak)
...
if (!in_array($bejovo_ertek, $mentendo_kodok)) {
    try {
        ...
        // A: Kettévágás (Split) - Ha a törlendő nap egy régi rekord közepébe vág bele
        $sql_split = "INSERT INTO `m_va_adatbazis` (`operátor_szám`, `sz_tp_kezdet`, `sz_tp_végzet`, `státusz`, `dokumentum_típusa`, `sz_tp_érték`, `jelentkezés_forrása`) 
                      SELECT `operátor_szám`, DATE_ADD(?, INTERVAL 1 DAY), `sz_tp_végzet`, `státusz`, `dokumentum_típusa`, `sz_tp_érték`, 'Kézi'
        ...
        // B: Szél-szűkítés (Shrink) balról és jobbról
        ...
        // C: Teljesen befedett rekordok törlése
        ...
        echo json_encode(['status' => 'ok', 'uzenet' => 'Terület felszabadítva ('.$bejovo_ertek.')']);
        exit;
    } catch (Exception $e) {

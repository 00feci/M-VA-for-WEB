...
document.addEventListener('click', function(e) {
    ...
    // JAVÍTÁS: Megállítjuk a buborékolást és a dupla lefutást
    e.stopPropagation();
    
    // Megnyitjuk a popupot
    window.nyisdMegAPopupot(td);
}, true);
// =========================================================
...
window.nyisdMegAPopupot = function(cella) {
   ...
   let tisztaTartalom = cella.textContent.replace(/[0-9]/g, '').trim(); 
    ...
    const szotar = { 'A': 'Rendszer Adat', 'SZ': 'Szabadság', 'TP': 'Táppénz', 'fn': 'Fiz. nélküli szabi' };
    
    let felsorolas = [];
    reszek.forEach(resz => {
        let nev = szotar[resz] || resz;
        let n = (resz === 'A') ? 1 : (cella.dataset.napok || 1);
        felsorolas.push(`<b>${nev} (${n} nap)</b>`);
    });
...
window.bezardAPopupot = function() {
...
window.generaldMiniNaptarat = function(fokuszNap, startLimit, endLimit, opKod) {
    ...
    const getCellContent = (n) => {
        ...
        return c ? c.textContent.replace(/[0-9]/g, '').trim() : '';
    };
    ...
    for (let i = startLimit; i <= endLimit; i++) {
        ...
        // Visszaállítva: Csak a nap száma, nagy kód és a nap típusa
        ...
        div.onclick = function() { 
            ...
            if (typeof window.frissitPopupNapokSzama === 'function') window.frissitPopupNapokSzama();
        };
...
window.popupMentese = function() {
    ...
    const manuálisNapok = napokMezo ? napokMezo.value : 1;
...
window.generaldMiniNaptarat_Global = function(fokuszNap, startLimit, endLimit) {
    ...
    for (let i = startLimit; i <= endLimit; i++) {
        ...
        div.innerHTML = `<div class="nap-szam" style="font-size: 11px; color: #666;">${String(i).padStart(2, '0')} ${napNev}</div><div class="nap-jelenlegi-kod" style="font-size: 18px; font-weight: bold; color: #333;">${aktualisKod}</div>`;
        if (i === fokuszNap) div.classList.add('kivalasztva');

...
function beirErtek(cell) {
  ...
  // Napok típusa sor frissítése
  cell.innerText = ertek;
  naptarFejlecMentese(cell, ertek);
  const napIndex = cell.cellIndex;
  const tabla = document.querySelector("table.munkaido");
  const sorok = tabla.rows;

  for (let i = 2; i < sorok.length; i++) {
    const adatSor = sorok[i];
    const celCella = adatSor.cells[napIndex];
    if (!celCella) continue;

    let szoveg = celCella.innerText.trim();
    ...
    if (ertek === 'M') {
      ...
      celCella.innerText = parts.join(' | ');
      continue;
    }

    // —— Ü vagy - logika ——
    const uj = ertek;    // "Ü" vagy "-"
    const masik = ertek === 'Ü' ? '-' : 'Ü'; // ami kizárandó

    let parts = [];
    if (szoveg) {
      parts = szoveg.split('|').map(p => p.trim()).filter(p => p);
    }

    // 1) töröljük a másik jelölést (Ü <-> -)
    parts = parts.filter(p => p !== masik);

    // 2) ha még nincs benne, hozzáadjuk
    if (!parts.includes(uj)) {
      parts.push(uj);
    }
...
// =========================================================
// 🏥 MODERN POPUP SZERKESZTŐ (CERUZA MÓD)
// =========================================================

let kivalasztottTipus = ''; // Itt tároljuk, mit választott (SZ, TP...)
let szerkesztoModAktiv = false; // Ez a "Ceruza" kapcsoló

// Ezt a függvényt hívd meg a Ceruza gombbal!
function toggleSzerkesztoMod() {
    ...
    // Vizuális visszajelzés a gombon (ha van)
    const btn = document.getElementById('btnSzerkesztoMod');
    if(btn) {
        btn.innerHTML = szerkesztoModAktiv ? '✏️ Szerkesztés: BE' : '👁️ Csak olvasás';
        btn.style.background = szerkesztoModAktiv ? '#4caf50' : '#ccc';
    }
}

// KATTINTÁS KEZELŐ (A táblázat celláira)
document.addEventListener('click', function(e) {
    // Csak ha aktív a szerkesztő mód!
    if (!szerkesztoModAktiv) return;

    const td = e.target.closest('td');
    // Csak adat-cellákra (ahol van data-nap és data-op)
    if (!td || !td.dataset.nap || !td.dataset.op) return;
    
    // Ha inaktív nap, ne csináljunk semmit
    if (td.classList.contains('inaktiv-nap')) return;
...
function nyisdMegAPopupot(cella) {
    ...
    // Név kikeresése (opcionális, de szép)
    let nev = opKod;
    ...
    // HTML Generálása (Ha még nincs a DOM-ban, létrehozzuk)
    ...
    if (!overlay) {
        ...
        overlay.innerHTML = `
            <div class="popup-doboz">
                ...
                <div style="font-weight:bold; margin-top:10px;">📅 Érintett napok kijelölése:</div>
                ...
                <div class="popup-footer">
                    <button class="btn-reset" onclick="popupReset()">♻️ Visszaállítás (Törlés)</button>
    ...
    // Tartalom frissítése
    document.getElementById('popupCim').innerText = `Javítás: ${nev}`;
    
    // Eredeti adat megjelenítése (Badgekkel)
    const eredetiSzoveg = cella.innerText; // Pl. "A | TP"
    const eredetiKontener = document.getElementById('popupEredetiAdatok');
    eredetiKontener.innerHTML = 'Jelenleg: ';
    if(!eredetiSzoveg) eredetiKontener.innerHTML += '<i>(Üres)</i>';
    else {
        // Szétbontjuk a | jeleknél
        eredetiSzoveg.split('|').forEach(tag => {
            const t = tag.trim();
            if(!t) return;
            let szinClass = 'badge-fn';
            if(t==='SZ') szinClass='badge-sz';
            if(t==='TP') szinClass='badge-tp';
            if(t==='A')  szinClass='badge-a';
            eredetiKontener.innerHTML += `<span class="adat-badge ${szinClass}">${t}</span>`;
        });
    }

    // MINI NAPTÁR GENERÁLÁSA
    ...
    // Metaadatok mentése a gombra (hogy tudjuk kinek mentünk)
...
function generaldMiniNaptarat(fokuszNap) {
    ...
    for (let i = 1; i <= napokSzama; i++) {
        ...
        // Ünnep/Hétvége detektálása a fejlécből
        // A cellIndexek el vannak csúszva (0: op, 1: név, 2: 1. nap)
        let napTipus = ''; 
        ...
        // Ha ünnep/hétvége, akkor legyen inaktív? (A te kérésed szerint)
        // Döntsd el: 'disabled' osztályt adunk neki, vagy csak hagyjuk pirosnak.
        // Én most csak pirosra hagyom, de ha akarod, vedd ki a kommentet:
        // if(napTipus) div.classList.add('disabled'); 

        // Alapból bepipáljuk azt, amire rákattintott
        if (i === fokuszNap) {
        ...
        // Kattintás (Többes kijelölés)
        div.onclick = function() {
            if(div.classList.contains('disabled')) return; // Ha tiltva van
...
// --- MENTÉS (A FŐ ATTRAKCIÓ) 💾 ---
function popupMentese() {
    if (!kivalasztottTipus) {
        alert("Kérlek válassz egy típust (SZ, TP...)!");
        return;
    }
    ...
    // Összeszedjük a kijelölt napokat
    const kijeloltNapok = [];
    document.querySelectorAll('#popupMiniNaptar .nap-box.kivalasztva').forEach(box => {
        kijeloltNapok.push(box.dataset.nap);
    });

    if (kijeloltNapok.length === 0) {
        alert("Nincs kijelölve nap!");
        return;
    }

    console.log(`Mentés indítása: ${opKod}, Napok: ${kijeloltNapok}, Típus: ${kivalasztottTipus}`);

    // KÜLDÉS A SZERVERNEK (Ciklusban, minden napra külön!)
    // Mivel a PHP-nk egyelőre egy napot tud fogadni, itt JS-ben loopolunk.
    // Ez gyors, nem veszed észre.
    
    const igeretek = kijeloltNapok.map(nap => {
        const datumStr = `${window.AblakCfg.ev}-${String(window.AblakCfg.honap).padStart(2,'0')}-${String(nap).padStart(2,'0')}`;
        
        return fetch(`${window.AblakCfg.apiBase}munkaido_korrekcio_kezelo.php?action=save`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                op: opKod,
    ...
    Promise.all(igeretek).then(() => {
        bezardAPopupot();
        // Frissítjük a nézetet (csak az érintett embert!)
...
// --- VISSZAÁLLÍTÁS (RESET) ♻️ ---
function popupReset() {
    if(!confirm("Biztosan törlöd a korrekciókat a kijelölt napokról? (Az eredeti adat visszaáll)")) return;
    ...
    const igeretek = kijeloltNapok.map(nap => {
        ...
        return fetch(`${window.AblakCfg.apiBase}munkaido_korrekcio_kezelo.php?action=reset`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ op: opKod, datum: datumStr })
        });
    });

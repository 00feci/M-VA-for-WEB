...
try {
    if (empty($_FILES['ratoltes_file']) || $_FILES['ratoltes_file']['error'] !== UPLOAD_ERR_OK) {
        echo json_encode([
            'sikeres' => false,
    ...
    $tmpFile = $_FILES['ratoltes_file']['tmp_name'];
    ...
    // ---------- INSERT előkészítés ----------
    // Közvetlenül az SQL oszlopneveket használjuk:
    // `Operátor név`, `Dátum`, `Hasznos munkavégzés`, `betöltés_dátum`
    $stmt = $pdo->prepare("
    ...
    echo json_encode([
        'sikeres' => true,
        'uzenet'  => "Rátöltés kész, {$beszurtSorok} sor beszúrva."
    ]);
} catch (Exception $e) {

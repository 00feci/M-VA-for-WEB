...
// Az 'A' nincs benne, mert az törlést vált ki, hogy a rendszer-adat látszódjon!
$mentendo_kodok = ['SZ', 'TP', 'fn'];

// 1. ESET: TÖRLÉS / Rendszer Adat (A) - Terület felszabadítása
if (!in_array($bejovo_ertek, $mentendo_kodok)) {
    try {
        if (!isset($pdo)) { $pdo = csatlakozasSzerver2(); }
        
        // Meglévő rekordok szűkítése (Shrink), hogy ne tűnjenek el a szomszédos napok
